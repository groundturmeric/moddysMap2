...
let ringColor = "rgb(22, 98, 295)";
let legendFontColor = "#CCD4EC"
...
function drawCircles(geo, data) {
    ...
    fillScale = d3.scaleOrdinal(d3.schemeCategory10)
        .domain(branches)
        // .range(["#052e9d", "#01ca87", "#1a9fdc", "orange"])
        .range(["#052e9d", "#DBF227", "#1a9fdc", "orange"])
...
*/
    ...
    function drawMap(geo, data) {
        ...
        xAxis
            ...
            // .append("path")
            // .attr("class", "pyramids")
            // // .attr("d",`M${d.longitude - 5},${d.latitude} C${d.longitude},${d.latitude+10} ${d.longitude},${d.latitude+10} ${d.longitude+5},${d.latitude}` ) // shape of pyramids
            // .attr("d", `M0,0 C5,4 5,7 2,10 L0,8.5 L-2,10 C-5,7 -5,4 0,0`)
            // .style("stroke", function (d) {
            //     if (d.NRSRO == "N/A") {
            //         return "white";
            //     } else {
            ...
            // )
            // .attr("fill-opacity", 1)
            // .style("transform", `translate(${projection(d.longitude), projection(d.latitude)})`)
